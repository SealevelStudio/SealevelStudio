        ...
        const scanData = {
          opportunities: opportunities.map((opp) => ({
            ...
            path: opp.steps.map((step) => ({
              ...
              inputToken: step.inputToken?.mint || step.inputToken?.symbol || 'Unknown',
              outputToken: step.outputToken?.mint || step.outputToken?.symbol || 'Unknown',
              inputAmount: Number(opp.inputAmount) / 1e9,
              outputAmount: Number(opp.outputAmount) / 1e9,
            })),
